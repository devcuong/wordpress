			...
			if (count($widgets) > 0) {
				foreach ($widgets as $id=>$name) {
					register_sidebar( array(
						'name'          => $name,
						'id'            => $id,
						'before_widget' => '<aside id="%1$s" class="widgetWrap widget %2$s">',
						'after_widget'  => '</aside>',
		...
		function themerex_admin_register_plugins() {
			...
			$plugins = array(
				...
				array(
					'name' 		=> 'Dsidxpress',
					'slug' 		=> 'dsidxpress',
					'source'	=> themerex_get_file_dir('/plugins/dsidxpress.zip'),
					'required' 	=> false
				)
			);
...
function themerex_init_template() {
	...
	global $logo_text, $logo_slogan, $logo_image, $logo_side, $logo_fixed, $logo_footer;
	...
	if (($logo_image = get_custom_option('logo_top')) == '' && file_exists(themerex_get_file_dir('/skins/' . esc_attr($skin) . '/images/logo-top.png')))
		$logo_image = themerex_get_file_url('/skins/' . esc_attr($skin) . '/images/logo-top.png');

	if ($logo_image=='' && ($logo_image = get_custom_option('logo_image')) == '' && file_exists(themerex_get_file_dir('/skins/' . esc_attr($skin) . '/images/logo.png')))
		$logo_image = themerex_get_file_url('/skins/' . esc_attr($skin) . '/images/logo.png');

	if (($logo_side = get_custom_option('logo_side')) == '' && file_exists(themerex_get_file_dir('/skins/' . esc_attr($skin) . '/images/logo-side.png')))
		$logo_side = themerex_get_file_url('/skins/' . esc_attr($skin) . '/images/logo-side.png');
	if ($logo_side=='') $logo_side = $logo_image;

	if (($logo_fixed = get_custom_option('logo_fixed')) == '' && file_exists(themerex_get_file_dir('/skins/' . esc_attr($skin) . '/images/logo-fixed.png')))
		$logo_fixed = themerex_get_file_url('/skins/' . esc_attr($skin) . '/images/logo-fixed.png');
	if ($logo_fixed=='') $logo_fixed = $logo_image;

	if (($logo_footer = get_custom_option('logo_image_footer')) == '' && file_exists(themerex_get_file_dir('/skins/' . esc_attr($skin) . '/images/logo-footer.png')))
		$logo_footer = themerex_get_file_url('/skins/' . esc_attr($skin) . '/images/logo-footer.png');
...
if ( !function_exists( 'themerex_add_widget_number' ) ) {
	function themerex_add_widget_number($prm) {
		...
		$num++;
		$prm[0]['before_widget'] = str_replace(' class="', ' class="widget-number-'.esc_attr($num).' ', $prm[0]['before_widget']);
		...
		if ($last_sidebar == 'top') {
			...
			if ($last_sidebar == 'footer') {
				...
				$prm[0]['before_widget'] = str_replace(' class="', ' class="columns1_'.esc_attr($columns).' ', $prm[0]['before_widget']);
				if ($num==1)
					$prm[0]['before_widget'] = '<div class="columnsWrap">' . balanceTags($prm[0]['before_widget']);
	...
	function themerex_callback_send_contact_form() {
		...
		if (!($contact_email = get_theme_option('contact_email')) && !($contact_email = get_theme_option('admin_email')))
			...
			if ($type=='call') {
				...
				$user_phone = themerex_substr($post_data['phone'], 0, 60);
				$user_msg = getShortString($post_data['message'], 300);

				$subj = sprintf(__('Site %s - Request call back from %s', 'themerex'), get_bloginfo('site_name'), $user_name);
				$msg = "\n".__('Name:', 'themerex')   .' '.$user_name
					.  "\n".__('Phone:', 'themerex') .' '.$user_phone
				...
				$user_msg = getShortString($post_data['message'], 300);
				$user_date = themerex_substr($post_data['date'], 0, 60);
				$user_time = themerex_substr($post_data['time'], 0, 60);

				$subj = sprintf(__('Site %s - Request Free Consultation %s', 'themerex'), get_bloginfo('site_name'), $user_name);
				$msg = "\n".__('Name:', 'themerex')   .' '.$user_name
					.  "\n".__('E-mail:', 'themerex') .' '.$user_email
					.  "\n".__('Message:', 'themerex').' '.$user_msg
					.  "\n".__('Date:', 'themerex').' '.$user_date
					.  "\n".__('Time:', 'themerex').' '.$user_time;
			}
			else if ($type=='okorder') {
