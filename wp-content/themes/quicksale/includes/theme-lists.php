...
if ( !function_exists( 'getColorScheme' ) ) {
	function getColorScheme($prepend_inherit=false) {
		$list = array();
		if ($prepend_inherit) $list['inherit'] = __("Inherit", 'themerex');
		$list['original']    = __('Original',  'themerex');
		$list['scheme_1']   = __('Scheme 1', 'themerex');
		$list['scheme_2']   = __('Scheme 2',   'themerex');
		$list['scheme_3']   = __('Scheme 3',  'themerex');
		return $list;
	}
}
...
if ( !function_exists( 'getThemeFontsList' ) ) {
	function getThemeFontsList($prepend_inherit=false) {
		...
		$list['Noto Serif'] = array('family'=>'serif');
