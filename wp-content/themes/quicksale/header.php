...
<body <?php
	...
	body_class('themerex_body ' . esc_attr($body_style) . ' top_panel_' . esc_attr($show_top_panel) . ' theme_skin_' . get_custom_option('theme_skin')
...
>
					...
					<div class="sc_scroll_slide swiper-slide">
						...
						<div class="logo<?php echo esc_attr($logo_text ? ' with_text' : ''); ?>">
							<a href="<?php echo esc_url(home_url()); ?>">
								<?php echo ( $logo_side ? '<img src="'.esc_url($logo_side).'" class="logo_side" alt="">' : ''); ?><?php echo (esc_attr($logo_text) ? '<span class="logo_text">'.apply_filters('theme_logo_text', $logo_text, 'sidemenu').'</span>' : ''); ?>
								<?php echo esc_html($logo_slogan) ? '<span class="logo_slogan">'.balanceTags($logo_slogan).'</span>' : ''; ?>
							</a>
						</div>
	...
	<div class="main_content">
		...
		<div class="boxedWrap">
			...
			?>
			<header class="noFixMenu <?php echo get_custom_option('menu_center')=='yes' ? 'menu_center' : 'menu_left'; ?> <?php echo get_custom_option('show_user_menu')!='yes' ? 'without' : 'with'; ?>_user_menu">
				<div class="topWrapFixed"></div>
				<div class="topWrap <?php
				if(get_custom_option('menu_style')=='line')	echo "styleShaded";
				else echo "styleFon"; ?>" <?php echo esc_attr($header_style); ?>>
						...
						<div class="usermenu_area">
							<div class="main">
								...
								<div class="menuUsItem menuItemLeft">
									<?php
									get_template_part('templates/page-part-user-panel-top');
									?>
								</div>
							</div>
						</div>
					...
					<div class="mainmenu_area">
						...
						<div class="wrap_logo">
							<div class="logo <?php echo esc_attr($logo_text ? ' with_text' : ''); ?>">
								<a href="<?php echo esc_url(home_url()); ?>">
									<?php echo (esc_attr($logo_image) ? '<img src="'.esc_url($logo_image).'" class="logo_main" alt=""><img src="'.esc_url($logo_fixed).'" class="logo_fixed" alt="">' : ''); ?>
									<?php if(esc_attr($logo_text) || esc_attr($logo_slogan)){ ?>
										<div class="logo_info">
											<?php echo (esc_attr($logo_text) ? '<span class="logo_text">'.apply_filters('theme_logo_text', $logo_text, 'mainmenu').'</span>' : ''); ?>
											<?php echo esc_attr($logo_slogan) ? '<span class="logo_slogan">'.esc_html($logo_slogan).'</span>' : ''; ?>
										</div>
									<?php } ?>
								</a>
							</div>
							...
							<div class="wrap_menu">
								...
								<?php if ((get_custom_option('show_contact_phone')=='yes' && get_custom_option('contact_info_phone')!='') || (get_custom_option('show_contact_location')=='yes' && get_custom_option('contact_info_location')!='')) { ?>
									<div class="infoTopWrap">
										<?php if (get_custom_option('show_contact_location')=='yes' && get_custom_option('contact_info_location')!='') { ?>
											<div class="location">
												<span class="info_icon icon-house"></span>
												<?php echo '<span class="contact_info_location">'.get_custom_option('contact_info_location').'</span>'; ?>
											</div>
										<?php } ?>

										<?php if (get_custom_option('show_contact_phone')=='yes' && get_custom_option('contact_info_phone')!='') { ?>
											<div class="phone">
												<span class="info_icon icon-phone-handle"></span>
												<?php echo '<span class="contact_info_phone">'.get_custom_option('contact_info_phone').'</span>'; ?>
											</div>
										<?php } ?>
									</div>
								<?php } ?>
								...
								<?php if (get_custom_option('menu_center')=='no'){ ?>
						...
						<?php if (get_custom_option('menu_center')=='yes'){ ?>
