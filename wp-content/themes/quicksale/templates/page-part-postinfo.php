...
if ($show_all || in_array('likes', $postinfo_buttons)) { ?>
	...
	?>
	<li class="likeButton like<?php echo esc_attr($allow ? '' : 'Active'); ?>" data-postid="<?php echo esc_attr($post_data['post_id']); ?>" data-likes="<?php echo esc_attr($post_data['post_likes']); ?>" data-title-like="<?php _e('Like', 'themerex'); ?>" data-title-dislike="<?php _e('Dislike', 'themerex'); ?>"><a class="icon-heart" title="<?php echo sprintf($allow ? __('Like - %s', 'themerex') : __('Dislike', 'themerex'), $post_data['post_likes']); ?>" href="#"><span class="likePost"><?php echo /*$post_data['post_likes']*/__('Like', 'themerex'); ?></span></a></li>
<?php
}
