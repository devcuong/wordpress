<div class="swpRightPos">
	...
	<div class="sc_tabs" data-active="<?php echo esc_attr($tab); ?>">
		...
		if (get_theme_option('show_theme_customizer') == 'yes') {
			$color_scheme = get_custom_option('color_scheme');
			if (empty($color_scheme)) $color_scheme = 'original';
			$color_scheme_list = themerex_get_list_color_schemes();
			$theme_skin = get_custom_option('theme_skin');
			$theme_color   = themerex_filter_get_color_1_theme(get_custom_option('theme_color'));
			$theme_color_1 = themerex_filter_get_color_2_theme(get_custom_option('theme_color_1'));
			$theme_color_2 = themerex_filter_get_color_3_theme(get_custom_option('theme_color_2'));
							...
							<div class="sc_scroll_slide swiper-slide">
								...
								<div class="co_options">
									<form name="co_form">
										...
										<div class="co_form_row">
											<span class="co_label"><?php _e('Color scheme:', 'themerex'); ?></span>
											<div class="co_form_subrow">

												<div id="co_scheme_list" data-options="color_scheme">
													<?php
													foreach($color_scheme_list as $k=>$v) {
														$scheme = themerex_get_file_url('skins/'.esc_attr($theme_skin).'/images/schemes/'.esc_attr($k).'.png');
														?>
														<a href="#" id="scheme_<?php echo esc_attr($k); ?>" class="co_scheme_wrapper<?php echo esc_attr($color_scheme==$k ? ' active' : ''); ?>" style="background-image: url(<?php echo esc_url($scheme); ?>)" data-value="<?php echo esc_attr($k); ?>" title="<?php echo esc_attr($v); ?>"></a>
													<?php
													}
													?>
												</div>
										...
										<div class="co_form_row">
											...
											<div id="co_bg_pattern_list">
												<?php for ($i=1; $i<=3; $i++) { ?>
												<a href="#" id="pattern_<?php echo esc_attr($i); ?>" class="<?php echo esc_attr($i==3 || $i==6 ? 'no_margin ' : ''); ?>co_pattern_wrapper<?php echo esc_attr($bg_pattern==$i ? ' current' : ''); ?>"><img src="<?php echo themerex_get_file_url('/images/bg/pattern_'.esc_attr($i).'_thumb2.png'); ?>" alt="" /></a>
												<?php } ?>
											</div>
										</div>
										<div class="co_form_row">
											...
											<div id="co_bg_images_list">
												<?php for ($i=1; $i<=3; $i++) { ?>
												<a href="#" id="image_<?php echo esc_attr($i); ?>" class="<?php echo esc_attr($i==3 || $i==6 ? 'no_margin ' : ''); ?>co_image_wrapper<?php echo esc_attr($bg_image==$i ? ' current' : ''); ?>"><img src="<?php echo themerex_get_file_url('/images/bg/image_'.esc_attr($i).'_thumb2.jpg'); ?>" alt="" /></a>
												<?php } ?>
											</div>
