					...
					<section class="masonry grid portfolioNOspacing <?php echo get_custom_option('show_filters')=='yes' ? 'isotope' :  'isotopeNOanim'; ?>">
						...
						if (!$post_number) {
							...
							if (get_custom_option('show_filters')=='yes') {
								if (get_custom_option('filter_taxonomy')=='categories') {			// Use categories as filter items
									...
									if (count($portfolio_list) > 0) {
										...
										foreach ($portfolio_list as $cat) {
											$filters .= '<li class="squareButton'.($cat->term_id==$cat_id ? ' active' : '').'"><a href="#" data-filter=".flt_'.esc_attr($cat->term_id).'">'.esc_html($cat->name).'</a></li>';
										}
									}
								...
								if ($filters) {
									$filters = '<ul>' . balanceTags($filters) . '</ul>';
									?>
				...
				if (in_array($pagination, array('viewmore', 'infinite'))) {
					if ($page_number < $wp_query->max_num_pages) {
						...
						<div id="viewmore" class="squareButton pagination_<?php echo esc_attr($pagination); ?>">
							<a href="#" id="viewmore_link" class="theme_button view_more_button"><span class="icon-spin3 viewmore_loading"></span><span class="viewmore_text_1"><?php _e('View more', 'themerex'); ?></span><span class="viewmore_text_2"><?php _e('Loading ...', 'themerex'); ?></span></a>
							<script type="text/javascript">
