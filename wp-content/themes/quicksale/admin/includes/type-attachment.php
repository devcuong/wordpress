	...
	if (isset($form_fields['media_folder'])) {
		...
		if (count($terms) > 0) {
			foreach ($terms as $slug=>$name) {
				...
				$html .= ($html ? '<br />' : '') . '<input type="checkbox" class="text" id="media_folder_'.esc_attr($id).'" name="media_folder_' . esc_attr($slug) . '" value="' . esc_attr( $slug ) . '"' . (in_array($slug, $values) ? ' checked="checked"' : '' ) . ' ' . esc_attr($readonly) . ' ' . esc_attr($aria_required) . ' /><label for="media_folder_'.$id.'"> ' . $name . '</label>';
			}
		}
