...
class WP_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		@set_time_limit(max(10, min(600, (int) get_theme_option('admin_dummy_timeout'))));
		$this->start_time = time();
		$this->max_time = round(ini_get('max_execution_time')*0.66);	// Break import after 2/3 of time limit
...
if ( $this->debug ) { printf( __( 'Author "%s" imported.', 'themerex' ).'<br>', esc_html($this->authors[$old_login]['author_display_name']) ); flush(); }
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			// Save into log last
			$result = round($posts_counter / $posts_all * 100);
		...
		if ( $id && ! is_wp_error( $id ) ) {
			if ($this->overwrite) {
				...
				if ($item['post_id'] != $id) {
					...
					$wpdb->query($wpdb->prepare("UPDATE " . esc_sql($wpdb->postmeta) . " SET meta_value=%d WHERE post_id=%d AND meta_key='_menu_item_menu_item_parent' AND meta_value=%d LIMIT 1", $item['post_id'], $item['post_id'], $id));
					if ($_menu_item_type == 'custom') {
						$wpdb->query($wpdb->prepare("UPDATE " . esc_sql($wpdb->postmeta) . " SET meta_value=%d WHERE post_id=%d AND meta_key='_menu_item_object_id' AND meta_value=%d LIMIT 1", $item['post_id'], $item['post_id'], $id));
					}
