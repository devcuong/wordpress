...
class TRX_Importer {
	...
	var $options = array(
		...
		'file_with_content'	=> array(
			'no_vc'	=> '/admin/tools/importer/data/dummy_data.xml',					// Path to the file with demo content without VC wrappers
			'vc'	=> '/admin/tools/importer/data/dummy_data_vc.xml'				// Path to the file with demo content for Visual Composer
			),
		'file_with_options'	=> '/admin/tools/importer/data/theme_options.txt',		// Path to the file with theme options
		'file_with_postmeta'=> '/admin/tools/importer/data/theme_postmeta.txt',		// Path to the file with post meta
		'file_with_widgets'	=> '/admin/tools/importer/data/widgets.txt',			// Path to the file with widgets data

		'domain_dev'		=> '.dnw',						// Domain on developer's server
		'domain_demo'		=> '.com',						// Domain on demo-server
		'demo_url'          => 'http://quicksale.themerex.net/', // URL of the demo site - need for change URLs in the custom menu items
	...
	function TRX_Importer() {
		...
		$this->import_log = dirname( __FILE__ ) . '/importer.log';
			...
			<?php if (empty($this->success) && $this->options['enable_importer']) { ?>
				<div class="trx_importer_section"<?php echo ($after_importer ? ' style="display:none;"' : ''); ?>>
					...
					<form id="trx_importer_form" action="#" method="post">
						...
						<p>
							<input type="radio" <?php echo !empty($this->options['overwrite_content']) ? 'checked="checked"' : ''; ?> value="overwrite" name="importer_action" id="importer_action_over" /><label for="importer_action_over"><?php _e('Overwrite existing content', 'themerex'); ?></label><br>
						...
						<p>
							...
							if (!empty($this->options['file_with_content']['vc']) && file_exists(themerex_get_file_dir($this->options['file_with_content']['vc']))) {
								?>
								<input type="radio" <?php echo ($this->options['data_type']=='vc' ? $checked : ''); ?> value="vc" name="data_type" id="data_type_vc" /><label for="data_type_vc"><?php _e('Import data for edit with Visual Composer', 'themerex'); ?></label><br>
								<?php
	...
	function importer() {
		...
		<p>&nbsp;</p>
		<div class="error">
			<h4><?php echo sprintf(__('Import progress: %s.', 'themerex'), ($this->last_slider > 0 ? 99 : $this->result).'%'); ?></h4>
		...
		if ($result >= 100 && empty($this->last_slider))
			...
			$this->error = '<h4>' . sprintf(__('Import progress: %s.', 'themerex'), ($this->last_slider > 0 ? 99 : $result).'%') . '</h4>'
	...
	function checkRequiredPlugins() {
		$not_installed = '';
		if (in_array('visual_composer', $this->options['required_plugins']) && $_POST['data_type']=='vc' && !class_exists('Vc_Manager') )
			$not_installed .= '<br>Visual Composer plugin';
		if (in_array('woocommerce', $this->options['required_plugins']) && !class_exists('Woocommerce'))
			$not_installed .= '<br>WooCommerce plugin';
		if (in_array('revslider', $this->options['required_plugins']) && !revslider_exists())
		...
		if (in_array('instagram', $this->options['required_plugins']) && is_plugin_inactive('wp-instagram-widget/wp-instagram-widget.php'))
	...
	function import_sliders() {
		...
		if (file_exists(WP_PLUGIN_DIR.'/revslider/revslider.php')) {
			...
			$dir = themerex_get_folder_dir('/admin/tools/importer/data/revslider');
			if ( is_dir($dir) ) {
				$hdir = @opendir( $dir );
				if ( $hdir ) {
					...
					while (($file = readdir( $hdir ) ) !== false ) {
						...
						if ( substr($file, 0, 1) == '.' || is_dir( trim($dir) . '/' . trim($file) ) || $pi['extension']!='zip' )
							continue;
						if ($this->options['debug']) printf(__('Slider "%s":', 'themerex'), $file);
						if (!is_array($_FILES)) $_FILES = array();
						$_FILES["import_file"] = array("tmp_name" => trim($dir) . '/' . trim($file));
