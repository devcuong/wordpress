...
function themerex_options_prepare_js($override='') {
	...
	<script type="text/javascript">
		...
		var THEMEREX_OPTIONS_export_list= [<?php
			if (($export_opts = get_option('themerex_options_'.esc_attr($override), false)) !== false) {
	...
	foreach ($themerex_options_data as $field) {
		...
		if (themerex_options_is_group($field['type'])) {					// Close nested containers
			...
			if (empty($field['end'])) {
				if (!$nesting) {
					if ($field['type']==$type) {
						$tabs .= '<li id="'.esc_attr($field['id']).'">'
							. '<a id="'.esc_attr($field['id']).'_title"'
							. ' href="#'.esc_attr($field['id']).'_content"'
...
function themerex_options_menu_list($field, $clone_val) {
	...
	foreach ($field['options'] as $key => $item) {
		...
		if (in_array($field['type'], array('list', 'select', 'fonts')))
			...
			$list .= '<span style="background-image:url('.esc_url($item).')" data-src="'.esc_attr($item).'" data-icon="'. esc_attr($key) .'" class="themerex_options_input_image"></span>';
		$list .= '</span>';
	}
	$list .= '</div>';
...
function themerex_options_action_button($data, $type) {
	...
	$output = '<span class="'
		...
		. '>'
		. ($type == 'button' || (empty($data['icon']) && !empty($data['title'])) ? esc_html($data['title']) : '')
		. '</span>';
...
function themerex_options_show_field($field, $value=null) {
	...
	if (themerex_options_is_group($field['type'])) {					// Close nested containers
		...
		if ($field['type'] != 'hidden') {
			echo ('<div class="themerex_options_field'
				...
				. $divider
				. '">'."\n");
			echo ('<label class="themerex_options_field_label'.(!empty($THEMEREX_flags['add_inherit']) && isset($field['std']) ? ' themerex_options_field_label_inherit' : '').'" for="' . esc_attr($field['id']) . '">' . esc_html($field['title'])
				. (!empty($THEMEREX_flags['add_inherit']) && isset($field['std']) ? '<span id="' . esc_attr($field['id']) . '_inherit" class="themerex_options_button_inherit'.($inherit ? '' : ' themerex_options_inherit_off').'" title="' . __('Unlock this field', 'themerex') . '"></span>' : '')
				. '</label>'
				. "\n");
			echo ('<div class="themerex_options_field_content'
				. esc_attr($buttons_classes)
	...
	foreach ($field['val'] as $clone_num => $clone_val) {
		...
		switch ( $field['type'] ) {
			...
			case 'group':
				echo ('<fieldset id="' . esc_attr($field['id']) . '" class="themerex_options_container themerex_options_group themerex_options_content'.esc_attr($divider).'">');
			...
			case 'toggle':
				...
				echo ('<div id="' . esc_attr($field['id']) . '" class="themerex_options_container themerex_options_toggle'.esc_attr($divider).'">');
				echo ('<h3 id="' . esc_attr($field['id']) . '_title"'
			...
			case 'accordion':
				...
				echo ('<div id="' . esc_attr($field['id']) . '" class="themerex_options_accordion_item">'
					. '<h3 id="' . esc_attr($field['id']) . '_title"'
					...
					. '</h3>'
					. '<div id="' . esc_attr($field['id']) . '_content" class="themerex_options_content themerex_options_accordion_content">');
				break;
			...
			case 'tab':
				...
				echo ('<div id="' . esc_attr($field['id']) . '_content"  class="themerex_options_content themerex_options_tab_content">');
				break;
			...
			case 'partition':
				...
				if (!empty($field['start']))
					echo ('<div id="' . esc_attr($field['start']) . '" class="themerex_options_container themerex_options_partition'.esc_attr($divider).'">'
						. '<ul>' . themerex_options_collect_tabs($field['type'], $field['start']) . '</ul>');
				echo ('<div id="' . esc_attr($field['id']) . '_content" class="themerex_options_content themerex_options_partition_content">');
				break;

			case 'hidden':
				echo ('<input class="themerex_options_input themerex_options_input_hidden" name="'. esc_attr($field['id']) .'" id="'. esc_attr($field['id']) .'" type="hidden" value="'. htmlspecialchars(is_inherit_option($clone_val) ? '' : $clone_val) . '" />');
				break;

			case 'date':
				if (isset($field['style']) && $field['style']=='inline') {
					echo ('<div class="themerex_options_input_date" id="'. esc_attr($field['id']) .'_calendar"'
						...
						. '></div>'
						. '<input id="'. esc_attr($field['id']) .'"'
						. ' name="'. esc_attr($field['id']) . ($cloneable ? '[]' : '') .'"'
						. ' type="hidden"'
			...
			case 'spinner':
				echo ('<input class="themerex_options_input themerex_options_input_spinner' . (!empty($field['mask']) ? ' themerex_options_input_masked' : '')
					...
					. (!empty($field['increment']) ? ' data-increment="'.esc_attr($field['increment']).'"' : '')
					. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. ' />'
			...
			case 'tags':
				if (!is_inherit_option($clone_val)) {
					...
					if (count($tags) > 0) {
						foreach($tags as $tag) {
							...
							echo ('<span class="themerex_options_tag iconadmin-cancel">'.balanceTags($tag).'</span>');
						}
					}
				}
				echo ('<input class="themerex_options_input_tags"'
					...
					. ' value=""'
					. ' />'
					. '<input name="'. esc_attr($field['id']) . ($cloneable ? '[]' : '') .'"'
					. ' type="hidden"'
					. ' value="'. htmlspecialchars(is_inherit_option($clone_val) ? '' : $clone_val) . '"'
					. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. ' />');
			...
			case "checkbox":
				echo ('<input type="checkbox" class="themerex_options_input themerex_options_input_checkbox"'
					...
					. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. ' />'
					. '<label for="' . esc_attr($field['id']) . '" class="' . (!empty($field['disabled']) ? 'themerex_options_state_disabled' : '') . ($clone_val=='true' ? ' themerex_options_state_checked' : '').'"><span class="themerex_options_input_checkbox_image iconadmin-check"></span>' . (!empty($field['label']) ? esc_html($field['label']) : esc_html($field['title'])) . '</label>');
				break;
			...
			case "radio":
				foreach ($field['options'] as $key => $title) {
					echo ('<span class="themerex_options_radioitem">'
						...
						. ' id="' . esc_attr($field['id']) . '_' . esc_attr($key) . '"'
						. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
						. ' />'
						. '<label for="' . esc_attr($field['id']) . '_' . esc_attr($key) . '"'. ($clone_val == $key ? ' class="themerex_options_state_checked"' : '') .'><span class="themerex_options_input_radio_image iconadmin-circle-empty'.($clone_val == $key ? ' iconadmin-dot-circled' : '') . '"></span>' . esc_html($title) . '</label></span>');
				}
				break;
			...
			case 'media':
				...
				if (!empty($clone_val) && !is_inherit_option($clone_val)) {
					...
					$ext = isset($info['extension']) ? balanceTags($info['extension']) : '';
					echo ('<a class="themerex_options_image_preview" rel="prettyPhoto" target="_blank" href="'.esc_url($clone_val).'">'.(!empty($ext) && themerex_strpos('jpg,png,gif', $ext)!==false ? '<img src="'.esc_url($clone_val).'" alt="" />' : '<span>'.esc_html($info['basename']).'</span>').'</a>');
				}
				break;
			...
			case 'range':
				echo ('<div class="themerex_options_input_range" data-step="'.(!empty($field['step']) ? esc_attr($field['step']) : 1).'">');
				...
				if (themerex_strpos($field['std'], $themerex_options_delimiter)!==false && themerex_strpos($clone_val, $themerex_options_delimiter)===false)
					$clone_val = $field['min'].','.esc_attr($clone_val);
				...
				echo ('<span class="themerex_options_range_min">'.esc_html($field['min']).'</span><span class="themerex_options_range_max">'.esc_html($field['max']).'</span>');
				echo ('<input name="'. esc_attr($field['id']) . ($cloneable ? '[]' : '') .'"'
					. ' type="hidden"'
					. ' value="' . htmlspecialchars(is_inherit_option($clone_val) ? '' : esc_attr($clone_val)) . '"'
					. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. ' />');
				echo ('</div>');
				break;
			...
			case 'socials':
				...
				if (empty($field['style']) || $field['style']=='icons') {
					list($after, $class) = themerex_options_action_button(array(
						...
						'icon' => (empty($field['style']) || $field['style']=='icons') && !empty($clone_val['icon']) ? esc_attr($clone_val['icon']) : 'iconadmin-users-1'
					), 'after');
				} else
					$after = '';
				echo ('<input class="themerex_options_input themerex_options_input_text themerex_options_input_socials'
					...
					. ' name="' . esc_attr($field['id']) . ($cloneable ? '[]' : '') .'"'
					. ' id="'. esc_attr($field['id']) .'"'
					. ' type="text" value="'. htmlspecialchars(is_inherit_option($clone_val['url']) ? '' : esc_attr($clone_val['url'])) . '"'
					. (!empty($field['mask']) ? ' data-mask="'.esc_attr($field['mask']).'"' : '')
					. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. ' />'
					. $after);
				...
				echo ('<input name="'. esc_attr($field['id']) . '_icon' . ($cloneable ? '[]' : '') .'" type="hidden" value="'. htmlspecialchars(is_inherit_option($clone_val['icon']) ? '' : esc_attr($clone_val['icon'])) . '" />');
				break;
			...
			case "color":
				echo ('<input class="themerex_options_input themerex_options_input_color'.(isset($field['style']) && $field['style']=='custom' ? ' themerex_options_input_color_custom' : '').'"'
					...
					. ' id="'. esc_attr($field['id']) . '"'
					. ' type="text"'
					. ' value="'. (is_inherit_option($clone_val) ? '' : esc_attr($clone_val)) . '"'
					. (!empty($field['action']) ? ' onchange="themerex_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. ' />');
		...
		if ($cloneable) {
			echo ('<input type="hidden" name="'. esc_attr($field['id']) . '_numbers[]" value="' . esc_attr($clone_num) . '" />'
				. '</div>');
		}
	}
	...
	if (!themerex_options_is_group($field['type']) && $field['type'] != 'hidden') {
		...
		if (!empty($THEMEREX_flags['add_inherit']) && isset($field['std']))
			echo  ('<div class="themerex_options_content_inherit"'.($inherit ? '' : ' style="display:none;"').'><div>'.__('Inherit', 'themerex').'</div><input type="hidden" name="' . esc_attr($field['id']) . '_inherit" value="'.esc_attr($inherit).'" /></div>');
		echo ('</div>');
		if (!empty($field['desc']))
			echo ('<div class="themerex_options_desc">' . balanceTags($field['desc']) .'</div>' . "\n");
		echo ('</div>' . "\n");
	}
}
...
function themerex_options_save() {
	...
	if ($mode=='export') {
		...
		$key = $name=='' ? $name2 : $name;
		$export = get_option('themerex_options_'.esc_attr($override), array());
		$export[$key] = $custom_options;
		if ($name!='' && $name2!='') unset($export[$name2]);
		update_option('themerex_options_'.esc_attr($override), $export);
...
function themerex_options_import() {
	...
	if (!empty($text)) {
		...
		$import = get_option('themerex_options_'.esc_attr($override), array());
