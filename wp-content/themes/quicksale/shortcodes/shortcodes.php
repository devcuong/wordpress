...
/*
[trx_button id="unique_id" type="square|round" fullsize="0|1" style="global|dark" size="medium|big|banner" icon="icon-name" link='#' target='']Button caption[/trx_button]
*/
...
function sc_contact_form($atts, $content = null) {
	...
	if($style == 'call') {
		...
		return '<div class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_call_form sc_contact_form_' . ($content ? 'custom' : 'contact') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		...
		. ($content != ''
			...
			. '<input placeholder="' . __('Phone', 'themerex') . '" class="sc_contact_form_phone" type="text" name="phone">'
			. '<div class="message">'
			. '<textarea placeholder="' . __('Message', 'themerex') . '" class="sc_contact_form_message" name="message"></textarea>'
			. '</div>'
			. '<div class="sc_contact_form_button">'
			. '<div class="squareButton sc_button_style_accent_2 sc_button_size_big global big"><a href="#" class="sc_contact_form_submit">' . __('Submit', 'themerex') . '</a></div>'
			. '</div>'
		)
		...
		return '<div class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_boxed_form sc_contact_form_' . ($content ? 'custom' : 'contact') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		. ($title ? '<h4 class="title">' . $title . '</h4>' : '')
		. ($description ? '<div class="description">' . $description . '</div>' : '')
		...
		themerex_enqueue_script( 'jquery-autosize', themerex_get_file_url('/js/jquery.autosize.js'), array('jquery'), null, true );
		return '<div class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_contact_form_' . ($content ? 'custom' : 'contact') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		...
		. ($content != ''
			...
			. '</div>'
			. '<div class="sc_contact_form_button">'
			. '<div class="squareButton sc_button_style_accent_2 sc_button_size_big global big"><a href="#" class="sc_contact_form_submit">' . __('Send the message', 'themerex') . '</a></div>'
			. '</div>'
		)
		. '<div class="result sc_infobox"></div>'
		. '</form>'
...
function sc_content($atts, $content=null){	
	...
	$s .= ($color !== '' ? 'color:' . $color . ';' : '');
	$s .= (sc_param_is_on($uppercase) ? 'text-transform:uppercase;letter-spacing:1px;' : '');
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$class .= (sc_param_is_on($uppercase) ? ' uppercase_on' : '');
	$class .= ($align=='left' ? ' sc_alignleft' : ($align=='right' ? ' sc_alignright' : ($align=='center' ? ' sc_aligncenter' : '')));
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
		. ' class="sc_content' .(sc_param_is_on($main) ? '' : ' main').''. ($class ? ' ' . $class : '') . '' .(sc_param_is_on($styling) ? ' sc_shadow box_style' : '').(sc_param_is_on($text_styling) ? ' text_styling' : '').'"'
		. ($s!='' ? ' style="'.$s.'"' : '').'>' 
		. do_shortcode($content) 
		. '</div>';
	return $output;
...
function sc_countdown($atts, $content = null) {
	...
	$s = getStyleString($top, $right, $bottom, $left, $width, $height);
	$s .= ($align && $align!='none' ? 'text-align:'.$align.';' : '');
	$c = ($style == 'flip' ? ' sc_countdown_flip' : ' sc_countdown_square');
	$c .= getStyleMargin($top, $right, $bottom, $left);
	...
	return '<div '.($id ? ' id="'.$id.'"' : '').'class="sc_countdown '.$c.'"'.($s ? ' style="'.$s.'"' : '').'><div class="sc_countdown_counter" data-style="'.($style == 'flip' ? 'flip' : 'square').'" data-date="'.$date.'" data-time="'.$time.'"></div></div>';
}
...
function sc_emailer($atts, $content = null) {
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	return '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation sFocus' . ($align && $align!='none' ? ' sc_align' . $align : '') . '' . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . '>'
		. '<h5>'.__('Submit', 'themerex').' <span>'.__('Newsletter', 'themerex').'</span>'.'</h5>'
		. '<form><input type="text" class="sInput" name="email" value="" placeholder="'.__('Email', 'themerex').'"></form>'
		. '<span class="sc_emailer_button mail sc_button sc_button_style_gray sc_button_size_big squareButton gray big" data-group="'.($group ? $group : __('E-mail collector group', 'themerex')).'">'
		. '<a href="#" title="'.__('Submit', 'themerex').'">'
		.  __('Subscribe', 'themerex')
		. '</a>'
		. '</span>'
		. '</div>';
}
...
function sc_icon($atts, $content=null){	
	...
	$s2 = ($weight != '' ? 'font-weight:'. $weight.';' : '')
		...
		. ($color != '' ? 'color:'.$color.';' : '')
		. ($bg_color != '' ? 'background-color:'.$bg_color.';' : '')
		. ($background == 'round' && (int) $size > 0 ? ($s ? '' : 'display:inline-block;') . 'width:' . round($size*2) . 'px;height:' . round($size*2) . 'px;line-height:' . round($size*2) . 'px;' : '')
	;
	return $icon!='' 
...
function sc_image($atts, $content=null){	
	...
	if (sc_param_is_on($link)){
		$s = getStyleString('!'.$top, '!'.$right, '!'.$bottom, '!'.$left);
		$s .= ($width != '' ? 'max-width:'.$width.'px;' : '').''.($height != '' ? 'max-height:'.$height.'px;' : '');
	} else {
		$s = getStyleString('!'.$top, '!'.$right, '!'.$bottom, '!'.$left, $width, $height);
	}
	...
	if(sc_param_is_on($link) && !empty($src) && !empty($url_color)){
	...
	else if(sc_param_is_on($link) && !empty($src) && empty($url_color)){
		return ('<a href="'.esc_url(trim($href)).'"' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image sc_image_hover_link' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
...
function sc_section($atts, $content=null){	
	...
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$class .= (sc_param_is_on($border_top) ? ' style_border_top' : '');
	$class .= (sc_param_is_on($border_bottom) ? ' style_border_bottom' : '');
	$s = getStyleString('!'.$top, '!'.$right, '!'.$bottom, '!'.$left, $width, $height)
		.($color !== '' ? 'color:' . $color . ';' : '')
		.($bg_color !== '' ? 'background-color:' . $bg_color . ';' : '')
		.($bg_image !== '' ? 'background-image:url(' . $bg_image . ');' : '')
		.($bg_image !== '' && sc_param_is_on($bg_cover) ? 'background-size: cover;;' : '')
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
		. ' class="sc_section'
			. (sc_param_is_on($pan) ? ' sc_shadow box_style pan_in' : '')
			. ($class ? ' ' . $class : '') 
...
function sc_slider($atts, $content=null){	
	...
	$output = ($border!='none' 
				? '<div class="sc_border sc_border_slider sc_border_'.$border.($border='light' ? ' sc_shadow' : '').''.($align!='' && $align!='none' ? ' sc_align'.$align : '').''. (!empty($class) ? ' '.$class : '').'"'.($ms.$hs ? ' style="'.$ms.$hs.'"' : '').'>'
				: '')
		...
		return '<div class="columns1_'.$THEMEREX_sc_team_count . (!empty($class) ? ' '.$class : '').'">'
				. '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_team_item sc_team_item_' . $THEMEREX_sc_team_counter . ($THEMEREX_sc_team_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_team_counter == 1 ? ' first' : '') . '">'
					. '<div class="sc_team_item_avatar">'
						. $photo
					.(!empty($link) ? '<a class="hoverLink" href="'.$link.'"></a>' : '')
					. '</div>'
...
function sc_testimonials($atts, $content=null){	
	...
	return
		'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_testimonials sc_testimonials_style'.(sc_param_is_off($controls) ? '' : ' sc_testimonials_controls_'.$controls) . (!empty($class) ? ' '.$class : '') .'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			.($title ? '<h2 class="sc_title sc_title_underline color_1 sc_testimonials_title">'.$title.'</h2>' : '')
