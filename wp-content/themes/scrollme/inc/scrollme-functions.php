<?php
    ...
    function scrollme_slide_clients() {
        ...
        <div class="s-panel-inner">
            <div class="container">
                <header class="entry-header">
                    <?php $section_title = scrollme_allow_span(get_theme_mod('scrollme_client_title', 'What we have done - <span>Our Works</span>')); ?>
                    <h1 class="entry-title"><?php echo $section_title; ?></h1>
                </header><!-- .entry-header -->
                <?php
...
class Menu_Attibute_Walker extends Walker_Nav_Menu
{
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if ( ( $hash_pos = strpos( $item->url,'#' ) ) !== false ) {
            $data_anchor = substr( $item->url, $hash_pos+1  );
        /*  if( ( $slash_pos = strpos( $data_anchor, '/') ) ) {
                $data_anchor = substr( $data_anchor, strpos( $data_anchor, +1  );
            }
        */
